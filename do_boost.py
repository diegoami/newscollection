...
def create_regressor(train_DF, train_file_aug, xboost_model_file):
...
def create_classifier(train_DF):
  # train_df = pd.read_csv(train_file, index_col=0)
    ...
    result_columns = 'SCO_USER'
    X_train = np.array(train_df[relevant_columns])
    y_train = np.array(train_df[result_columns].map(lambda x: 1 if x > 0.75 else 0))
    xgbparams = {
        #'learning_rate':[ 0.001, 0.005, 0.01, 0.05, 0.1],
        #'n_estimators' : [100,200,500]
        #'max_depth' : [2,3,4], 'min_child_weight' : [5,6,7]
...
if __name__ == '__main__':
   ...
   # create_regressor(train_df, train_file_aug, xboost_model_file)
    create_classifier(train_df)