...
relevant_columns =  [ 'SCO_DAYS',
                      'SCO_D_TEXT', 'SCO_D_TITLE',   'SCO_D_SUMMARY', 'SCO_D_SUMMARY_2',
                      'SCO_T_TEXT','SCO_T_TITLE', 'SCO_T_SUMMARY', 'SCO_T_SUMMARY_2',  'SCO_CW_TITLE',  'SCO_CW_TEXT', 'SCO_CW_SUMMARY', 'SCO_CW_SUMMARY_2' ]
...
def create_classifier(train_DF, xboost_classifier_file ):
 ...
 #
    ...
    xgbparams = {
        ...
        #'reg_alpha' : [0, 0.1, 0.001, 0.0001]
       # 'learning_rate' : [0.1, 0.01, 0.001], 'n_estimators' : [200, 300, 400]

    }
   # clf =  GridSearchCV(XGBRegressor(max_depth=4, min_child_weight=5, gamma=0,colsample_bytree=0.8,subsample=0.8, reg_alpha=0.01, learning_rate=0.1), xgbparams)
  #  clf =  GridSearchCV(XGBClassifier(max_depth=4, min_child_weight=5, gamma=0, subsample=0.7,colsample_bytree=0.6, reg_alpha=0.1,learning_rate= 0.01, n_estimators=300 ), xgbparams)


    clf = XGBClassifier(max_depth=4, min_child_weight=5, gamma=0, subsample=0.7,colsample_bytree=0.6, reg_alpha=0.1,learning_rate= 0.01, n_estimators=300 )


    clf.fit(X_train, y_train)


    print_performance(X_train, clf, y_train)
...
def print_performance(X_train, clf, y_train):
