...
SELECT
  ...
  FROM ((((`tnaggregator_phrases`.`PREDICTIONS` `P` LEFT JOIN `tnaggregator_phrases`.`ARTICLE_INFO` `A1`
      ...
      ON ((`P`.`PRED_AIN_ID_2` = `A2`.`AIN_ID`))) LEFT JOIN `tnaggregator_phrases`.`CLASSIF` `C`
      ON (((`C`.`CLAS_AIN_ID_1` = `P`.`PRED_AIN_ID_1`) AND (`C`.`CLAS_AIN_ID_2` = `P`.`PRED_AIN_ID_2`)))) LEFT JOIN
    (SELECT
       avg(`tnaggregator_phrases`.`SAME_STORY_USER`.`SSU_SIMILARITY`) AS `SCORE`,
     ...
     GROUP BY `tnaggregator_phrases`.`SAME_STORY_USER`.`SSU_AIN_ID_1`,
