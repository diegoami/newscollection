...
CREATE TABLE `TRAINING_MODEL` (
  ...
  `TMO_YCLF_MEAN` float DEFAULT NULL,
  `TMO_YREG_MEAN` float DEFAULT NULL,
  `TMO_ROC_AUC` float DEFAULT NULL,
  `TMO_NAME` varchar(32) DEFAULT NULL,
  PRIMARY KEY (`TMO_ID`)
) ;


CREATE TABLE `SPIDER_REPORT` (
  `SRE_ID` int(11) NOT NULL AUTO_INCREMENT,
  `SRE_DATE` datetime DEFAULT NULL,
  `SRE_REPORT` varchar(256) DEFAULT NULL,
  PRIMARY KEY (`SRE_ID`)
);
...
CREATE TABLE `FEATURE_REPORT` (
  ...
  `FRE_REPORT` varchar(512) DEFAULT NULL,
  PRIMARY KEY (`FRE_ID`)
);
