...
from . import end_condition, build_text_from_paragraphs, extract_date
...
class CnbcSpider(TechControversySpider):
    ...
    def parse_page(self, response):
        ...
        all_paragraph_before = response.xpath("//div[@id='group']//li/text()").extract()
        all_paragraphs = response.xpath(
            "//div[contains(@class, 'ArticleBody-articleBody')]//p[not(.//aside) and not(.//twitterwidget) and not(.//figure) and not(.//h2)  and not(.//script) and not(.//div[@class=mid-banner-wrap])]//text()").extract()
